      ...
      try{
        ...
        if(!args[0].toLowerCase().includes("http"))
          return message.channel.send(new MessageEmbed()
            .setFooter(ee.footertext,ee.footericon)
            .setColor(ee.wrongcolor)
            .setTitle("❌ ERROR Wrong Command Usage | Include a Track URl | **A URL**").setDescription(`Try this: \`${prefix}setsongoftheday <Track_URL> <message>\`\n\nExample: \`${prefix}setsongoftheday https://www.youtube.com/watch?v=HCjNJDNzw8Y Today It's a masterpeace which gives you happyness and old feelings.\``)
          )
        ...
        if(!args[1])
          return message.channel.send(new MessageEmbed()
            .setFooter(ee.footertext,ee.footericon)
            .setColor(ee.wrongcolor)
            .setTitle("❌ ERROR Wrong Command Usage | Include a Message").setDescription(`Try this: \`${prefix}setsongoftheday <Track_URL> <message>\`\n\nExample: \`${prefix}setsongoftheday https://www.youtube.com/watch?v=HCjNJDNzw8Y Today It's a masterpeace which gives you happyness and old feelings.\``)
          )
          ...
          try {
              ...
              return message.channel.send(new MessageEmbed()
                .setColor(ee.wrongcolor)
                .setFooter(ee.footertext, ee.footericon)
                .setTitle(`❌ Error | There was an error while searching: ${args[0]}`)
          ...
          if(!res.tracks[0])
            return message.channel.send(new MessageEmbed()
              .setColor(ee.wrongcolor)
              .setFooter(ee.footertext, ee.footericon)
              .setTitle(String("❌ Error | Found nothing for: **`" + args[0]).substr(0, 256-3) + "`**")
              ...
              fs.writeFile("./botconfig/songoftheday.json", JSON.stringify(ss, null, 3), (e) => {
                  ...
                  return message.channel.send(new MessageEmbed().setFooter(ee.footertext,ee.footericon).setColor(ee.color).setTitle("✅ Successfully set the new Track of the Day"))
                });
