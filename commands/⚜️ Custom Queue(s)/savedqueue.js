    ...
    try{
      ...
      if(!Type)
        return message.channel.send(new MessageEmbed()
          ...
          .setTitle(`❌ ERROR | You didn't entered a TYPE`)
      ...
      switch (Type.toLowerCase()) {
        case "create": {
          if(!Name)
            return message.channel.send(new MessageEmbed()
              .setColor(ee.wrongcolor)
              .setFooter(ee.footertext, ee.footericon)
              .setTitle(`❌ ERROR | You didn't entered a Saved-Queue-Name`)
          ...
          if (Name.length > 10)
            return message.channel.send(new MessageEmbed()
              ...
              .setTitle("❌ Error | Your Saved-Queue-Name is too long!")
          ...
          if(client.queuesaves.get(message.author.id, `${Name}`))
            return message.channel.send(new MessageEmbed()
              .setFooter(ee.footertext, ee.footericon)
              .setColor(ee.wrongcolor)
              .setTitle("❌ Error | Your Queue already exists!")
        ...
        case "addcurrenttrack": {
          ...
          if(!client.queuesaves.get(message.author.id, `${Name}`))
            return message.channel.send(new MessageEmbed()
              .setFooter(ee.footertext, ee.footericon)
              .setColor(ee.wrongcolor)
              .setTitle("❌ Error | Your Queue does not exists yet!")
          ...
          //return susccess message
          return message.channel.send(new MessageEmbed()
            .setTitle(`✅ Success | Added ${track.title} onto the Queue \`${Name}\``.substr(0, 256))
        ...
        case "addcurrentqueue": {
          ...
          if (!tracks.length)
            return message.channel.send(new MessageEmbed()
              .setFooter(ee.footertext, ee.footericon)
              .setColor(ee.wrongcolor)
              .setTitle("❌ Error | The Queue is Empty!")
            );
          ...
          //return susccess message
          return message.channel.send(new MessageEmbed()
            .setTitle(`✅ Success | Added ${tracks.length} Tracks onto the Queue \`${Name}\``)
        ...
        case "removetrack": case "removesong": {
          ...
          if(!Options)
            return message.channel.send(new MessageEmbed()
              .setColor(ee.wrongcolor)
              .setFooter(ee.footertext, ee.footericon)
              .setTitle(`❌ ERROR | You didn't entered an Option (the Track you want to remove (ID OF IT))`)
          ...
          if(!client.queuesaves.get(message.author.id, `${Name}`))
            return message.channel.send(new MessageEmbed()
              .setFooter(ee.footertext, ee.footericon)
              .setColor(ee.wrongcolor)
              .setTitle("❌ Error | Your Queue is not existing!")
              .setDescription(`Create it with: \`${prefix}savedqueue create ${Name}\``)
            );
          ...
          if(Number(Options) >= tracks.length || Number(Options) < 0)
            return message.channel.send(new MessageEmbed()
              .setColor(ee.wrongcolor)
              .setFooter(ee.footertext, ee.footericon)
              .setTitle(`❌ ERROR | Your provided Option is out of Range (\`0\` - \`${tracks.length-1}\`)`)
              .setDescription(`See all your Tracks: \`${prefix}savedqueue showdetails ${Name}\`Usage: \`${prefix}savedqueue removetrack ${Name} <Song number>\``)
            )
          ...
          //return susccess message
          return message.channel.send(new MessageEmbed()
            .setTitle(`✅ Success | Deleted ${deletetrack.title} of the Queue \`${Name}\``.substr(0, 256))
            .setDescription(`There are now: \`${client.queuesaves.get(message.author.id, `${Name}`).length} Tracks\`\n\nPlay it with: \`${prefix}savedqueue play ${Name}\``)
            .setColor(ee.color)
            .setFooter(ee.footertext, ee.footericon)
          );
        ...
        case "shuffle": case "mix": {
          ...
          if(!Array.isArray(oldtracks) )
            return message.channel.send(new MessageEmbed()
              .setFooter(ee.footertext, ee.footericon)
              .setColor(ee.wrongcolor)
              .setTitle(`❌ Error | Your Saved-Queue ${Name} is Empty!`)
              .setDescription(`Add the current **Queue** onto it: \`${prefix}savedqueue addcurrentqueue ${Name}\`\nAdd the current **Track** onto it: \`${prefix}savedqueue addcurrenttrack ${Name}\``)
            );
          ...
          //return susccess message
          return message.channel.send(new MessageEmbed()
            .setTitle(`✅ Success | Shuffled ${newtracks.length} Tracks of the Queue \`${Name}\``.substr(0, 256))
            .setDescription(`There are now: \`${client.queuesaves.get(message.author.id, `${Name}`).length} Tracks\`\n\nPlay it with: \`${prefix}savedqueue play ${Name}\``)
            .setColor(ee.color)
            .setFooter(ee.footertext, ee.footericon))
        }
        ...
        case "removedupes": case "removeduplicates": {
          ...
          //return susccess message
          return message.channel.send(new MessageEmbed()
            .setTitle(`✅ Success | Removed ${counter} Tracks from the Queue \`${Name}\``.substr(0, 256))
            .setDescription(`There are now: \`${client.queuesaves.get(message.author.id, `${Name}`).length} Tracks\`\n\nPlay it with: \`${prefix}savedqueue play ${Name}\``)
            .setColor(ee.color)
            .setFooter(ee.footertext, ee.footericon))
        }
        ...
        case "showall": case "listall": case "show": case "queue": case "list": {
          ...
          if(Object.size(queues) <= 1)
            return message.channel.send(new MessageEmbed()
              .setColor(ee.wrongcolor)
              .setFooter(ee.footertext, ee.footericon)
              .setTitle(`❌ ERROR | You don't have any Queues saved yet`)
        ...
        case "createsave": case "cs": case "save": {
          ...
          //return susccess message
          return message.channel.send(new MessageEmbed()
            .setTitle(`✅ Success | Created ${Name} and Added ${tracks.length} Tracks to it`)
        ...
        case "delete": case "remove": case "del": {
          ...
          //return susccess message
          return message.channel.send(new MessageEmbed()
            .setTitle(`✅ Success | Deleted the Queue \`${Name}\``)
            .setColor(ee.color)
            .setFooter(ee.footertext, ee.footericon)
          )
        }
        ...
        case "play": case "load": case "p": case "add": case "paly": {
          ...
          if (!channel)
            ...
            if(!player)
              return message.channel.send(new MessageEmbed()
                .setFooter(ee.footertext, ee.footericon)
                .setColor(ee.wrongcolor)
                .setTitle("❌ Error | I am not connected to a Channel!")
                .setDescription(`Play a track first for example: \`${prefix}playsongoftheday\``)
              );
          ...
          .setColor(ee.color)
          .setFooter(ee.footertext, ee.footericon)
          .setTitle(`Attempting to Load ${client.queuesaves.get(message.author.id, `${Name}`).length} Tracks`)
          ...
          //return susccess message
          message.channel.send(new MessageEmbed()
            .setTitle(`✅ Success | Loaded ${client.queuesaves.get(message.author.id, `${Name}`).length} Tracks onto the current Queue`)
            .setColor(ee.color)
            .setFooter(ee.footertext, ee.footericon)
          )
        ...
        default:
          return message.channel.send(new MessageEmbed()
            .setColor(ee.wrongcolor)
            .setFooter(ee.footertext, ee.footericon)
            .setTitle(`❌ ERROR | You didn't entered a **valid** TYPE`)
            .setDescription(`Usage: \`${prefix}savedqueue <Type> <Name>\`\nValid Types:\n\`create\`, \`addcurrenttrack\`, \`addcurrentqueue\`, \`removetrack\`, \`removedupes\`, \`showall\`, \`createsave\`, \`delete\`, \`showdetails\`, \`play\`, \`shuffle\``)
          );
        break;
