    ...
    try{
      ...
      if (channel.id !== player.voiceChannel)
        ...
        for(let i = 0; i < limit; i++){
          ...
          .setFooter(ee.footertext, ee.footericon)
          .setColor(ee.color)
          .addField("**0) CURRENT TRACK**", `[${player.queue.current.title.substr(0, 35)}](${player.queue.current.uri}) - \`${player.queue.current.isStream ? "LIVE STREAM" : format(player.queue.current.duration).split(" | ")[0]}\` - request by: **${player.queue.current.requester.tag}**`)
          .setDescription(desc))
          ;
        }
