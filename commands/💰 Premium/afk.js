    ...
    try{
      ...
      if(args[0].toLowerCase() === "guild") {
        ...
        return message.channel.send(new MessageEmbed()
          .setColor(ee.color)
          .setFooter(ee.footertext, ee.footericon)
          .setTitle(`💰 Successfully ${player.get(`afk-${message.guild.id}`) ? `✅ Enabled` : `❌ Disabled`} 24/7`)
      ...
      //if the args 0 is user
        ...
        if(message.author.id !== player.get("playerauthor"))
          return message.channel.send(new MessageEmbed()
            .setColor(ee.wrongcolor)
            .setFooter(ee.footertext, ee.footericon)
            .setTitle("❌ Error | You did not created that player!")
        ...
        return message.channel.send(new MessageEmbed()
          .setColor(ee.color)
          .setFooter(ee.footertext, ee.footericon)
          .setTitle(`💰 Successfully ${player.get(`afk-${player.get("playerauthor")}`) ? `✅ Enabled` : `❌ Disabled`} 24/7`)
